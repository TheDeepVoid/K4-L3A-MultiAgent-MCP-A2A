...
class LocalEmbeddingIndex:
    ...
    def build_from_clean(self) -> "LocalEmbeddingIndex":
        """Build the configured baseline collection from the clean JSON artifact."""
    ...
    def search(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        if self.collection is None:
            raise RuntimeError("The index has not been built or loaded yet.")
    ...
    def semantic_search(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        """Compatibility alias for the phase 3 smoke-test command."""
